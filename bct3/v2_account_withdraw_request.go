...
// 转账（取现）接口
// https://docs.baofu.com/docs/bct3/bct3-3002-001-01
func AccountWithdrawRequest(config *BCT3Config, req *AccWithdrawReq) (*AccWithdrawResp, error) {
	// 服务编号
	serviceTp := SERVICE_ACCOUNT_WITHDRAW
	...
	resp := &AccWithdrawResp{}
	if err = tool.ParseJSON(response.Body, resp); err != nil {
		return nil, err
	}

	// 判断取现受理状态
	if resp.State == WITHDRAW_STATE_FAILURE {
		return nil, errors.Errorf("取现受理失败:%s", resp.TransRemark)
	}

...
// 协议支付- 查询绑卡状态
func AgreementQueryBindBankCard(config *AgreementPayConfig, req *AgreementQueryBindBankCardRequest) (map[string]string, error) {
	// 交易类型
	txnType := TRANS_TYPE_QUERY_BIND_REL
	...
	// 先BASE64后进行AES加密
	accNo, err := tool.AesEncrypt(tool.Base64Encode(req.AccNo), aesKey)
	if err != nil {
		return nil, err
	}
	...
	if resp["resp_code"] == RESP_CODE_SUCCESS {
		...
		// 解密账户信息中的敏感数据
		if resp["protocols"] != "" {
			protocols, _ := tool.AesDecrypt(resp["protocols"], rAesKey)
			resp["protocols"], _ = tool.Base64DecodeStr(protocols)
		}
	}
